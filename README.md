...
## 核心设计
1. 拆解标准化规则与高阶技术：将提示词工程标准规则和高阶技术拆分为独立参考文件，让 Agent 优先按照基础范式完成内容撰写，再根据具体场景灵活选用 Few-Shot、推理策略、ReAct 式工具调用、检索增强等高阶技术能力。
2. 初稿自检与优化迭代：完成内容初稿后，通过专属评估清单开展内部质量校验，在此基础上压缩冗余内容、合并重复规则、修正无法落地部署的内容，保障输出可用性。
3. 标准化领域适配机制：引入领域适配清单，让 Agent 依据目标用户、业务任务、资料来源、工具副作用、输出格式及安全边界完成精准的领域适配，杜绝通过项目名称主观猜测业务规则的问题。
4. 运行时细节规则化转译：将运行时底层实现细节，转化为 Agent 可直接执行的自然语言行为规则。例如将 Schema 字段、类名、上下游流程等技术术语，改写为目标 Agent 可精准理解的标准化行为指令。
5. 模块化能力组合设计：针对多类 Agent 核心能力提供模块化参考方案，涵盖检索增强、代码执行、客服支持、研究分析等场景，支持提示词按实际需求灵活组合能力模块，摒弃固定领域模板的僵化模式。
6. 量化验证与效果校验：配套专属评估脚本与完整测试用例，通过 with skill / without skill 对照实验，量化验证各类技能是否真正提升系统提示词的输出质量。
