...
3. 引入领域适配清单，让 agent 根据目标用户、任务、资料来源、工具副作用、输出格式和安全边界做领域适配，而不是凭项目名猜业务规则。
4. 把具体代码名词翻译成 agent 可执行的行为规则，例如把上下游流程词改写成目标 agent 能理解的自然语言规则。
5. 针对多类 agent 能力提供模块化参考，如检索增强、代码执行、客服支持、研究分析，使提示词能按能力组合，而不是套固定领域模板。
6. 配套评估脚本和测试用例，用使用 skill 和不使用 skill 对照验证 skill 是否真的改善系统提示词质量。
